...
no_dep_e.g.1 <- select(no_dep, -(year:day))

# You can be more explicit but it requires more typing

no_dep_e.g.2 <- select(no_dep, -year, -month, -day) 
...
no_dep_e.g.3 <- select(no_dep, day, month, year, 
                sched_dep_time, sched_arr_time, everything())

# Note the use of the helper function "everything()"
# Repeat the code without using it

no_dep_e.g.4 <- select(no_dep, day, month, year, 
...
no_dep_v2 <- select(no_dep, -dep_time, -dep_delay, -arr_time, -arr_delay,
...
delays <- dplyr::select(flights, contains("delay"))
