...
# were only able to touch on this evening such as colour scales, and 
...
plot_combined <- ggplot(dat, aes(x = CPI, y = HDI, color = Region)) +
    geom_smooth(mapping = aes(linetype = "r2"),
                method = "lm",
                formula = y ~ x + log(x), se = FALSE,
                color = "red") +
...
ggsave("outputs/corruption_development.png", width = 30, height = 20, units = "cm")
